...
#define iTime u_time
#define iMouse (u_mouse * u_resolution)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 m = vec2(iMouse.x / 5.0, iMouse.y / 2.0);
    vec2 mo = m / iResolution.xy-.5;
    mo = (mo==vec2(-.5))?mo=vec2(-0.1,0.1):mo;
    mo.x *= iResolution.x/iResolution.y;
    rd.yz *= mm2(mo.y);
    rd.xz *= mm2(mo.x + sin(u_time*0.05)*0.2);
